...
import cz.coffeerequired.shadowed.google.gson.JsonElement;
...
public class SaveCache extends AsyncEffect {
    private SaveCache.Mode mode;
    ...
    protected void execute(Event event) {
        ...
        if (mode.equals(SaveCache.Mode.SINGLE)) {
    ...
    public String toString(@Nullable Event event, boolean debug) {
        return "save " + (mode.equals(SaveCache.Mode.SINGLE) ? "storage id " + expressionId.toString(event, debug) : "all storages");
    }

    @SuppressWarnings("unchecked")
    @Override
    public boolean init(Expression<?>[] expressions, int matchedPattern, Kleenean isDelayed, SkriptParser.ParseResult parseResult) {
        mode = matchedPattern == 0 ? SaveCache.Mode.SINGLE : SaveCache.Mode.ALL;
        if (mode == SaveCache.Mode.SINGLE) {
